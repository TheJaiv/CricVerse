...
def get_smart_ai_bowler_odi(innings, pitch, weather="Clear", format_overs=50):
    ...
    if not pool: pool = [p for p in all_live if _main(p) and _quota(p)]
    if not pool: pool = [p for p in all_live if _quota(p) and _nc(p)]
    if not pool: pool = [p for p in all_live if _quota(p)]
    if not pool: pool = [p for p in all_live if _nc(p)] or all_live
...
def execute_ball_math_odi(match):
    ...
    if match.pitch == "Flat":
        ...
        bowl_rating += 5; bat_rating -= 2

    _new_ball = innings.total_balls < 90
    _mid_ball = innings.total_balls < 180
    ...
    if match.current_innings_num == 2 and not is_collapse:
        ...
        if balls_left > 0:
            ...
            if total_balls < 120:
                threshold = 8.0; max_p = 1.20; scale = 0.05
            elif total_balls < 210:
                threshold = 7.5; max_p = 1.35; scale = 0.08
            else:
                threshold = 6.5; max_p = 1.60; scale = 0.12
            if rrr > threshold:
    ...
    if "Bouncer" in deliv:
        ...
        if innings.bouncers_in_over == 2:
            is_no_ball = True; is_bouncer_no_ball = True
    ...
    if is_no_ball:
        ...
        innings.total_runs += 1
        bow_stats.runs_conceded += 1
        if not is_bouncer_no_ball:
            match.free_hit = True

    dot_weight = max(22.0, 50.0 - diff * 0.45)
    single_weight = 40.0
    boundary_weight = max(1.5, 7.0 + diff * 0.35)
    wicket_weight = max(1.0, 3.0 - diff * 0.15)

    if match.pitch == "Green" and "Pace" in bowler["role"]:
        wicket_weight *= 1.15; boundary_weight *= 0.90
    elif match.pitch == "Dusty" and "Spin" in bowler["role"]:
        wicket_weight *= 1.15; boundary_weight *= 0.85; dot_weight *= 1.10
    elif match.pitch == "Dry" and "Spin" in bowler["role"] and total_balls > 150:
        wicket_weight *= 1.10; dot_weight *= 1.05
    elif match.pitch == "Hard":
        if total_balls < 60 and "Pace" in bowler["role"]: wicket_weight *= 1.10
        else: boundary_weight *= 1.05
    elif match.pitch == "Soft":
        dot_weight *= 1.20; boundary_weight *= 0.80
    elif match.pitch == "Cracked":
        wicket_weight *= 1.20; boundary_weight *= 0.85
    elif match.pitch == "Damp":
        if "Pace" in bowler["role"] and total_balls < 90:
            wicket_weight *= 1.25; boundary_weight *= 0.80
    elif match.pitch == "Dead":
        boundary_weight *= 1.20; wicket_weight *= 0.85
    elif match.pitch == "Worn":
        if "Spin" in bowler["role"] and total_balls > 150:
            wicket_weight *= 1.20; dot_weight *= 1.10; boundary_weight *= 0.85
    elif match.pitch == "Turning":
        if "Spin" in bowler["role"]:
            wicket_weight *= 1.25; boundary_weight *= 0.75; dot_weight *= 1.15
    elif match.pitch == "Two-Paced":
        dot_weight *= 1.25; boundary_weight *= 0.80; wicket_weight *= 1.10
    elif match.pitch == "Slow":
        dot_weight *= 1.20; boundary_weight *= 0.75
        if "Spin" in bowler["role"]: wicket_weight *= 1.15
    elif match.pitch == "Bouncy":
        if "Pace" in bowler["role"]: wicket_weight *= 1.15
    elif match.pitch == "Sticky":
        wicket_weight *= 1.40; boundary_weight *= 0.65; dot_weight *= 1.35
    elif match.pitch == "Flat":
        boundary_weight *= 1.10; wicket_weight *= 0.95

    _new_ball = total_balls < 90
    _mid_ball = total_balls < 180
    if match.weather == "Overcast":
        ...
        if _new_ball and "Pace" in bowler["role"]: wicket_weight *= 1.12; boundary_weight *= 0.93
        elif _mid_ball and "Pace" in bowler["role"]: wicket_weight *= 1.05
    elif match.weather == "Humid":
        if _new_ball and "Pace" in bowler["role"]: wicket_weight *= 1.10
        elif _mid_ball and "Pace" in bowler["role"]: wicket_weight *= 1.04
    elif match.weather == "Dry Heat":
        dot_weight *= 1.10
    elif match.weather == "Windy":
        ...
        wicket_weight *= 0.85; boundary_weight *= 1.10
    elif match.weather in ["Heavy Rain", "Thunderstorm"]:
        wicket_weight *= 0.70; boundary_weight *= 1.25

    if is_powerplay:
        if "Pace" in bowler["role"]: wicket_weight *= 1.20
        boundary_weight *= 1.15; dot_weight *= 1.10
    elif is_middle:
        single_weight *= 1.35; dot_weight *= 0.85; boundary_weight *= 0.80
    ...
    if "Yorker" in deliv:
        ...
        if shot in SPIN_SHOT_MATRIX[deliv]: perfect_shot_selection = True
            ...
            boundary_weight *= 0.25; dot_weight *= 1.3; single_weight *= 1.1
    ...
    if shot in ["Block", "Defensive"]:
        ...
        if striker["archetype"] == "Aggressor":
            ...
            if b_stats.balls_faced >= 40 and (is_death_overs or pressure_multiplier > 1.15):
                boundary_weight *= 1.15; wicket_weight *= 1.05
            else:
    ...
    if "Mystery" in deliv:
        wicket_weight *= 1.6; dot_weight *= 1.5; boundary_weight *= 0.6; single_weight *= 0.8
    ...
    if shot in ["Loft", "Scoop"]:
        four_weight *= 0.6; six_weight *= 3.0
        if not is_death_overs: wicket_weight *= 3.0
        else: wicket_weight *= 1.5
        dot_weight *= 0.8
    elif shot in ["Block", "Defensive", "Leave"]:
        four_weight = 0.0; six_weight = 0.0
    elif shot in ["Drive", "Cut", "Pull", "Flick", "Sweep"]:
        four_weight *= 1.2; six_weight *= 0.4
    ...
    four_weight = max(0.5, min(four_weight, 25.0))
    six_weight = max(0.1, min(six_weight, 15.0))
    wicket_weight = max(1.0, min(wicket_weight, 25.0))
    ...
    if outcome == "wicket":
        ...
        if innings.wickets < max_wickets:
            is_ai_batting = match.is_ai_game and match.get_striker_user_id() == getattr(match, "p2_id", None)
        ...
        if runs in [4, 6] and not is_bye:
            ...
            if runs == 4: b_stats.fours = getattr(b_stats, 'fours', 0) + 1
            elif runs == 6: b_stats.sixes = getattr(b_stats, 'sixes', 0) + 1
