...
def execute_ball_math_t20(match):
    ...
    _new_ball = innings.total_balls < 36
    ...
    if match.current_innings_num == 2:
        ...
        if balls_left > 0:
            ...
            if total_balls < 36:
                threshold = 10.0; max_p = 1.25; scale = 0.05
            elif total_balls < 90:
                threshold = 11.0; max_p = 1.35; scale = 0.06
            else:
                threshold = 10.5; max_p = 1.50; scale = 0.08
            if rrr > threshold:
    ...
    if is_cutter:
        ...
        if innings.cutters_in_over > 1 and random.random() < 0.50:
            ...
            if free_hit_active: match.last_commentary_prefix = "FH\n"
            return
    else:
        is_cutter = False
    ...
    single_weight = 40.0
    boundary_weight = max(2.0, 10.0 + diff * 0.45)
    wicket_weight = max(1.2, 5.0 - diff * 0.18)
    ...
    if match.pitch == "Flat":
        boundary_weight *= 1.05; wicket_weight *= 0.95
    elif match.pitch == "Green" and "Pace" in bowler["role"]:
        wicket_weight *= 1.10; boundary_weight *= 0.95
    elif match.pitch == "Dusty" and "Spin" in bowler["role"]:
        wicket_weight *= 1.10; boundary_weight *= 0.90; dot_weight *= 1.05
    elif match.pitch == "Dry" and "Spin" in bowler["role"] and innings.total_balls > 60:
        wicket_weight *= 1.05; dot_weight *= 1.05
    elif match.pitch == "Hard":
        if innings.total_balls < 36 and "Pace" in bowler["role"]: wicket_weight *= 1.05
        else: boundary_weight *= 1.05
    elif match.pitch == "Soft":
        dot_weight *= 1.10; boundary_weight *= 0.90
    elif match.pitch == "Cracked":
        wicket_weight *= 1.15; boundary_weight *= 0.90
    elif match.pitch == "Damp":
        if "Pace" in bowler["role"] and innings.total_balls < 36:
            wicket_weight *= 1.15; boundary_weight *= 0.85
    elif match.pitch == "Dead":
        boundary_weight *= 1.15; wicket_weight *= 0.90
    elif match.pitch == "Worn":
        if "Spin" in bowler["role"] and innings.total_balls > 60:
            wicket_weight *= 1.15; dot_weight *= 1.05; boundary_weight *= 0.90
    elif match.pitch == "Turning":
        if "Spin" in bowler["role"]:
            wicket_weight *= 1.15; boundary_weight *= 0.85; dot_weight *= 1.10
    elif match.pitch == "Two-Paced":
        dot_weight *= 1.15; boundary_weight *= 0.85; wicket_weight *= 1.05
    elif match.pitch == "Slow":
        dot_weight *= 1.15; boundary_weight *= 0.85
        if "Spin" in bowler["role"]: wicket_weight *= 1.10
    elif match.pitch == "Bouncy":
        if "Pace" in bowler["role"]: wicket_weight *= 1.10
    elif match.pitch == "Sticky":
        wicket_weight *= 1.25; boundary_weight *= 0.75; dot_weight *= 1.20

    _new_ball = total_balls < 36
    if match.weather == "Overcast":
        wicket_weight *= (1.22 if _new_ball else 1.08); boundary_weight *= (0.85 if _new_ball else 0.93)
    elif match.weather == "Cloudy":
        if _new_ball and "Pace" in bowler["role"]: wicket_weight *= 1.10; boundary_weight *= 0.95
    elif match.weather == "Humid":
        if _new_ball and "Pace" in bowler["role"]: wicket_weight *= 1.08
    elif match.weather == "Dry Heat":
        dot_weight *= 1.10
    elif match.weather == "Windy":
        wicket_weight *= (1.15 if _new_ball else 1.07); boundary_weight *= 0.95
    elif match.weather in ["Light Rain", "Drizzle"]:
        wicket_weight *= 0.90; boundary_weight *= 1.05
    elif match.weather in ["Heavy Rain", "Thunderstorm"]:
        wicket_weight *= 0.80; boundary_weight *= 1.15
    ...
    if "Yorker" in deliv:
        ...
        if shot in SPIN_SHOT_MATRIX[deliv]: perfect_shot_selection = True
            ...
            boundary_weight *= 0.20; dot_weight *= 1.4; single_weight *= 1.1
    ...
    if shot in ["Block", "Defensive"]:
        ...
        if striker["archetype"] == "Aggressor":
            ...
            if b_stats.balls_faced >= 20 and (is_death_overs or pressure_multiplier > 1.15):
                boundary_weight *= 1.2; wicket_weight *= 1.05
            else:
    ...
    four_weight = max(0.5, min(four_weight, 35.0))
    six_weight = max(0.1, min(six_weight, 25.0))
    wicket_weight = max(1.0, min(wicket_weight, 30.0))
